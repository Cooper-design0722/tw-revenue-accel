...
def normalize_revenue(raw: list, market: str) -> list:
    ...
    for row in raw:
        ...
        if code is None or cur_yoy is None or cum_yoy is None:
            continue
        cur_rev = to_float(pick(row, "營業收入-當月營收"))
        prev_rev = to_float(pick(row, "營業收入-上月營收"))
        # 月增率優先用交易所自己給的欄位，沒有才用當月/上月自行計算
        mom = to_float(pick(row, "營業收入-上月比較增減(%)", "營業收入-上月比較增減％"))
        if mom is None and cur_rev is not None and prev_rev:
        ...
        out.append({
            ...
            "cur_revenue_k": cur_rev,
            "prev_revenue_k": prev_rev,
...
def load_history_lists(category: str) -> dict:
    """
    掃描 data/history/ 所有歷史檔，取出指定類別（目前為 'momentum'）
...
def compute_streaks(items: list, current_key: str, history: dict) -> None:
    """計算連續入選月數：往回追歷史存檔，連續在名單內就累加"""
    for r in items:
        streak = 1
        ...
        while key in history and r["code"] in history[key]:
            ...
            key = prev_ym(key)
        r["streak"] = streak
    max_streak = max((r["streak"] for r in items), default=0)
    multi = sum(1 for r in items if r["streak"] >= 2)
    print(f"     連續入選：最長 {max_streak} 個月，連2個月以上共 {multi} 家")
...
def main():
    ...
    # 自行篩選區用的完整名單：不做任何策略篩選，全市場個股都放進來，
    # 讓使用者自己在網頁上篩選或匯出 CSV 到 Excel 分析。
    # 欄位刻意精簡，2000 檔的 JSON 才不會過大拖慢頁面載入。
    all_stocks = [{
        "code": r["code"], "name": r["name"], "market": r["market"],
        "industry": r["industry"], "size_tier": r["size_tier"],
        "market_cap_k": r["market_cap_k"],
        "cur_revenue_k": r["cur_revenue_k"], "cum_revenue_k": r["cum_revenue_k"],
        "cur_yoy": r["cur_yoy"], "cum_yoy": r["cum_yoy"], "mom": r["mom"],
    } for r in rows]
    with_mom = sum(1 for r in all_stocks if r["mom"] is not None)
    print(f"\n=== 自行篩選區 ===")
    print(f"     輸出全市場 {len(all_stocks)} 檔，其中 {with_mom} 檔有月增率")
    ...
    payload = {
        ...
        "history_months": len(hist_momentum) + 1,
    ...
    print(f"      動能延續股 {len(momentum)} 家　自行篩選區 {len(all_stocks)} 檔")
