...
def build_industry_map() -> dict:
    """
    建立 公司代號 -> 產業別 的對照表。
    上市取自公司基本資料，上櫃取自月營收表（兩者都含產業別欄位）。
    """
    imap = {}
    for row in fetch_json(TWSE_CAPITAL_URL, "TWSE 上市公司基本資料(產業別)"):
        code = pick(row, "公司代號", "SecuritiesCompanyCode")
        ind = pick(row, "產業別", "Industry")
        if code and ind:
            imap[str(code).strip()] = str(ind).strip()
    for row in fetch_json(TPEX_REVENUE_URL, "TPEx 上櫃月營收(產業別)"):
        code = pick(row, "公司代號", "SecuritiesCompanyCode")
        ind = pick(row, "產業別", "Industry")
        if code and ind:
            imap.setdefault(str(code).strip(), str(ind).strip())
    print(f"     -> 產業別對照表 {len(imap)} 檔")
    return imap
